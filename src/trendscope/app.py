...
def _setup_logging():
    ...
    env = os.environ.get("ENV", os.environ.get("TRENDSCOPE_ENV", "development"))
...
CSUITE_SERVICE_TOKEN = os.environ.get("CSUITE_SERVICE_TOKEN", "")
...
async def lifespan(app: FastAPI):
    ...
    _service.get_scheduler().start()
    logger.info("TrendScope started (scheduler active)")
    yield
    _service.get_scheduler().stop()
...
async def landing():
    """Serve the TrendScope marketing landing page."""
    return FileResponse(os.path.join(_FRONTEND_DIR, "landing.html"))


@app.get("/dashboard", include_in_schema=False)
async def dashboard():
    """Serve the TrendScope dashboard SPA."""
    return FileResponse(os.path.join(_FRONTEND_DIR, "index.html"))

_STATIC_DIR = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "..", "frontend", "static"))
...
async def get_tenant(request: Request) -> dict:
    """Validate bearer token against Zuultimate, or accept internal service token.

    Internal services (Content Production, C-Suite) can bypass Zuultimate
    auth by sending X-Service-Token matching CSUITE_SERVICE_TOKEN.  They
    receive a synthetic tenant with full entitlements.
    """
    # Internal service token bypass
    service_token = request.headers.get("X-Service-Token", "")
    if service_token and CSUITE_SERVICE_TOKEN and hmac.compare_digest(service_token, CSUITE_SERVICE_TOKEN):
        return {
            "user_id": "service",
...
async def health_ready():
    """Readiness probe — checks service initialization and Zuultimate reachability."""
...
# ── C-Suite Priority endpoints ────────────────────────────────────────────────


def _validate_service_token(request: Request) -> None:
    """Validate X-Service-Token header against CSUITE_SERVICE_TOKEN."""
    if not CSUITE_SERVICE_TOKEN:
        raise HTTPException(status_code=503, detail="Service token not configured")
    token = request.headers.get("X-Service-Token", "")
    if not hmac.compare_digest(token, CSUITE_SERVICE_TOKEN):
        raise HTTPException(status_code=403, detail="Invalid service token")


@app.post("/v1/priority/refresh")
async def priority_refresh(request: Request):
    """Run refresh_trends() immediately — C-Suite priority bypass."""
    _validate_service_token(request)
    result = await _svc().refresh_trends()
    return {"status": "ok", "result": result}


@app.post("/v1/priority/analysis")
async def priority_analysis(request: Request):
    """Run autonomous analysis immediately — C-Suite priority bypass."""
    _validate_service_token(request)
    result = await _svc().run_autonomous_analysis()
    return {"status": "ok", "result": result}
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("trendscope.app:app", host="0.0.0.0", port=8002)
