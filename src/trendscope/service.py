...
Provides a clean API for C-Suite executives to interact with the
...
from trendscope.integrations.nexus_publisher import NexusPublisher
...
class TrendService:
    """
    High-level trend intelligence service for C-Suite executives.
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self._anomaly_detector = AnomalyDetector(self.db)
        self._snapshot_manager = SnapshotManager(self.db)
        self._lifecycle_tracker = LifecycleTracker(self.db)
        self._coverage_analyzer = CoverageAnalyzer(self.db)
        self._time_comparator = TimeComparator(self.db)
        self._feed = IntelligenceFeed()
        self._kh_sync = KHSync(self.db)
        ...
        self._autonomy_dashboard = AutonomyDashboard(self)

        # Register default schedules
        self._scheduler.register("refresh_trends", 60.0, self._sync_refresh_trends)
        self._scheduler.register("detect_anomalies", 120.0, self._sync_detect_anomalies)
    ...
    def _sync_refresh_trends(self) -> None:
        """Sync wrapper for async refresh_trends, used by the scheduler."""
        loop = self._loop
        if loop is not None and loop.is_running():
            future = asyncio.run_coroutine_threadsafe(self.refresh_trends(), loop)
            future.result(timeout=120)
        else:
            asyncio.run(self.refresh_trends())

    def _sync_detect_anomalies(self) -> None:
        """Sync wrapper for detect_anomalies (sync method, kept for consistency)."""
        self.detect_anomalies(lookback_days=14)
    ...
    async def initialize(self) -> None:
        ...
        # Capture the running event loop for scheduler sync wrappers
        try:
    ...
    async def refresh_trends(self, sources: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # Run analysis to update velocity, momentum, and credibility scores
        analysis_result = {}
        try:
            analysis_result = self.intelligence.analyze_all()
            logger.info(
                "Post-refresh analysis: %d trends analyzed, %d correlations",
                analysis_result.get("analyzed_trends", 0),
                analysis_result.get("correlations_found", 0),
            )
        except Exception as e:
            logger.warning("Post-refresh analysis failed: %s", e)

        # Publish top trends to Nexus knowledge base (fire-and-forget)
        if trends:
            asyncio.create_task(self._publish_top_trends_to_nexus(trends))

        return {
            "trends_collected": len(trends),
            "trends_analyzed": analysis_result.get("analyzed_trends", 0),
            "sources_used": sources or list(self.collector_manager.collectors.keys()),
            "refreshed_at": self._last_refresh.isoformat(),
        }

    async def _publish_top_trends_to_nexus(self, trends: list) -> None:
        """Publish top 10 trends to Nexus knowledge base (best-effort)."""
        try:
            top = sorted(trends, key=lambda t: getattr(t, "score", 0), reverse=True)[:10]
            for trend in top:
                await self._nexus_publisher.publish_trend_signal(
                    signal_name=getattr(trend, "name", str(trend)),
                    category=str(getattr(trend, "category", "general")),
                    analysis=getattr(trend, "description", "")[:500],
                    confidence=min(getattr(trend, "score", 50) / 100, 1.0),
                )
        except Exception as exc:
            logger.debug("Nexus trend publish failed: %s", exc)
